...
export default function TradeAnalysis90({ charts }: Props) {
  const analysisData = useMemo(() => {
    const results: TradeAnalysisData[] = [];

    // Filter trades to only include $0.90 trades
    const filteredTrades = myTrades.filter(trade => trade.price === 0.90);
    ...
    return (
      <div style={{
        ...
        <h3 style={{ color: '#f1f5f9', margin: '0 0 10px 0', fontSize: '16px' }}>
          Trade Analysis - $0.90 Trades (Realistic Prices ≥ $0.10)
        </h3>
        <p style={{ color: '#94a3b8', margin: 0 }}>
          No trades at $0.90 found with matching market data.
        </p>
      </div>
    );
  }
        ...
        {analysisData.map((trade, index) => (
          ...
          >
            ...
            <div style={{ 
              ...
              ID: {trade.assetId.slice(0, 8)}...{trade.assetId.slice(-8)}
            </div>
          </div>
        ))}
      </div>
